...
int main() {
  ...
  server.Get(R"(/hoho/(\d+)/(\d+))") = [] (int path_param_int, float path_param_float) {
    using json = nlohmann::json;
    json j;
  ...
  server.Get(R"(/yoho/(\d+))") = [] (int path_param, const QueryParams& query_params) {
    using json = nlohmann::json;
    json j;
  ...
  server.Get(R"(/numbers/(\d+)/numbers/(\d+))") = [] (const PathParams& path_params, const QueryParams& query_params){
